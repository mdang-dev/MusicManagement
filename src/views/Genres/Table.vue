...
function goToUpdate(userId) {
  router.push((routing.path + '/' + userId).replace('list', 'form'));
}
...
const filteredGenres = computed(() => {
   return genres.value.filter((genre) => genre?.genreName.toUpperCase().includes(searchValue.value.toUpperCase()));
});


function removeGenre(e) {
  genres.value =  genres.value.filter((genre) => genre.id !== e.currentTarget.getAttribute('data-id'));
  axios.delete(`http://localhost:4000/genres/${e.currentTarget.getAttribute('data-id')}`)
    .then(() => {
      fetchData();
    }).catch((error) => {
      console.log(error);
  ...
  <Layout>
    ...
    <div v-if="!isLoading" class="card mr-4">
      <div class="card-body">
        ...
        <ScrollAreaRoot
          class="w-full h-[75vh] rounded overflow-hidden shadow-md border border-gray-300">
          <ScrollAreaViewport class="w-full h-full">
            <table class="table">
              ...
              <tbody>
              <tr v-for="(genre, index) in filteredGenres" class="text-center">
                <th scope="row">{{ index + 1 }}</th>
                <td>{{ genre.id}}</td>
                <td>{{ genre.genreName }}</td>
                <td>
                  <div class="d-flex w-full gap-2 align-items-center justify-content-center">
                    <button type="button" class="btn btn-outline-primary" @click="goToUpdate(genre.id)"><i class="bi bi-pencil-square mr-1"></i>Chỉnh Sửa</button>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <button type="button" @click="removeGenre" data-id id="btn-toast" class="btn btn-primary btn-remove-genre" data-bs-dismiss="modal">Xác nhận</button>
          </div>
        </div>
      </div>
    </div>
